...
def start_worker(app, num, docker_run_kwargs, busy_wait=None):
    ...
    container_name = 'rmexp-sec6-{}-{}-{}'.format(
...
def start_feed(app, video_uri, tokens_cap, stop_after, exp='', client_id=0):
    ...
    exp_type = os.environ.get('EXP_TYPE')
    assert exp_type is not None, 'EXP_TYPE environ needs to be set'
    assert exp_type.lower() in [
        'baseline', 'cpushares'], 'EXP_TYPE must either be "baseline" or "cpushares", not {}'.format(exp_type)
    if exp_type == 'cpushares':
        client_type = device_type_map[app]
        dutycycle_sampling_on = dutycycle_sampling_on_map[app]
    elif exp_type == 'baseline':
        client_type = 'baseline'
    ...
    try:
        feed.start_single_feed_token(
            ...
            loop=True,
            random_start=True,  # different from sec 5 harness
            client_id=client_id,
...
def kill():
    ...
    ret = subprocess.check_output(
        "docker rm -f $(docker ps --filter 'name=rmexp-sec6-{}-*' -a -q)".format(os.getpid()), shell=True)
    logger.info(ret)
