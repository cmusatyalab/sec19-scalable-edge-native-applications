...
def normalize_color(img, mask=None, mask_info = None, mask_apply = None, method = 'hist', max_percentile = 100, min_percentile = 0):
    shape = img.shape
    ...
    if method == 'hist': # doesn't work well for over-exposed images
        ...
        mask_over_exposed = np.bitwise_and(zc.super_bitwise_or((mask_blue_over_exposed, mask_green_over_exposed, mask_red_over_exposed)), mask_over_bright)
...
@config.profile()
def _locate_board(img, stretch_ratio, display_list):
    ## Find some black
    DoB = zc.get_DoB(img, config.BLUR_KERNEL_SIZE, 1, method = 'Average')
    zc.check_and_display('DoB', DoB, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    mask_black = color_inrange(DoB, 'HSV', V_L = config.BLACK_DOB_MIN_V)
    zc.check_and_display('mask_black', mask_black, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
...
def _detect_lego(img_board, display_list, method = 'edge', edge_th = [80, 160], mask_black_dots = None, mask_lego_rough = None, add_color = True):
    if method == 'edge':
        ...
        zc.check_and_display('board_edge', edges, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
        edges = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, zc.generate_kernel(7, 'circular'), iterations = 1) # magic kernel size
        edges = cv2.morphologyEx(edges, cv2.MORPH_OPEN, zc.generate_kernel(3, 'square'), iterations = 1)
        ...
        if add_color:
            ...
            mask = mask_rough
        zc.check_and_display('edge_inv', mask, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)

    elif method == 'dots':
        mask = cv2.morphologyEx(mask_black_dots, cv2.MORPH_CLOSE, zc.generate_kernel(11, 'square'), iterations = 1) # magic kernel size
    ...
    mask_lego, _ = zc.find_largest_CC(mask, min_area = config.BOARD_RECONSTRUCT_AREA / 300.0, min_convex_rate = 0.2, ref_p = config.BOARD_RECONSTRUCT_CENTER, max_dist_ref_p = config.BOARD_RECONSTRUCT_PERI / 15.0)
...
def _find_lego(img, display_list, img_board, perspective_mtx, thickness):
    ...
    zc.check_and_display('board', img_board, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)

    #################### detect Lego on the board ##############################
    ## locate Lego approach 1: using edges with pre-normalized image, edge threshold is also pre-defined
    rtn_msg, img_lego_u_edge_S, mask_lego_u_edge_S = _detect_lego(img_board, display_list, method = 'edge', edge_th = [50, 100], add_color = False)
    if rtn_msg['status'] != 'success':
        return (rtn_msg, None)
    mask_lego_rough_L = zc.expand(mask_lego_u_edge_S, 21, method = 'circular', iterations = 2)
    mask_lego_rough_S = zc.expand(mask_lego_u_edge_S, 11, method = 'circular', iterations = 2)
    ...
    if 'board_grey' in display_list:
        ...
        zc.check_and_display('board_grey', img_board_grey, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    ...
    rtn_msg, img_lego_u_edge_S, mask_lego_u_edge_S = _detect_lego(img_board, display_list, method = 'edge', edge_th = edge_th, add_color = False)
    if rtn_msg['status'] != 'success':
        return (rtn_msg, None)
    zc.check_and_display('lego_u_edge_S', img_lego_u_edge_S, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)

    ## locate Lego approach 2: using edges with normalized image
    # the three steps below for color correction is not well studied, but it's here for historical reasons...
    ...
    zc.check_and_display('board_n0', img_board_n0, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    ...
    edge_th = [dynamic_range / 4 + 35, dynamic_range / 2 + 70]
    rtn_msg, img_lego_u_edge_norm_L, mask_lego_u_edge_norm_L = _detect_lego(img_board_n0, display_list, method = 'edge', edge_th = edge_th, add_color = True)
    if rtn_msg['status'] != 'success':
        return (rtn_msg, None)
    zc.check_and_display('lego_u_edge_norm_L', img_lego_u_edge_norm_L, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    ...
    img_board_tmp[mask_lego_bool, :] = (int(bw_board[mask_grey_bool].max()) + int(bw_board[mask_grey_bool].min())) / 2
    DoB = zc.get_DoB(img_board_tmp, 41, 1, method = 'Average')
    DoB[mask_lego_bool] = 0
    #DoB[mask_lego_rough_L_inv] = 0
    zc.check_and_display('board_DoB', DoB, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    mask_black = color_inrange(DoB, 'HSV', V_L = config.BD_DOB_MIN_V)
    zc.check_and_display('board_mask_black', mask_black, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    mask_black_dots, n_cnts = zc.get_small_blobs(mask_black, max_peri = config.BOARD_BD_MAX_PERI)
    if n_cnts < 1000: # some sanity check
        ...
        return (rtn_msg, None)
    zc.check_and_display('board_mask_black_dots', mask_black_dots, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)

    rtn_msg, img_lego_u_dots_L, mask_lego_u_dots_L = _detect_lego(img_board, display_list, method = 'dots', mask_black_dots = mask_black_dots, mask_lego_rough = mask_lego_rough_L, add_color = False)
    if rtn_msg['status'] != 'success':
        return (rtn_msg, None)
    zc.check_and_display('lego_u_dots_L', img_lego_u_dots_L, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    ...
    zc.check_and_display('board_n1', img_board_n1, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    zc.check_and_display('board_n2', img_board_n2, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    zc.check_and_display('board_n3', img_board_n3, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    zc.check_and_display('board_n4', img_board_n4, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    zc.check_and_display('board_n5', img_board_n5, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    zc.check_and_display('board_n6', img_board_n6, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    ...
    mask_green = zc.super_bitwise_and((mask_green, mask_green_n1, mask_green_n3, mask_lego_u_edge_norm_L))
    mask_yellow = zc.super_bitwise_and((mask_yellow, mask_yellow_n1, mask_yellow_n3, mask_lego_u_edge_norm_L))
    mask_red = zc.super_bitwise_and((mask_red, mask_red_n1, mask_red_n3, mask_lego_u_edge_norm_L))
    mask_blue = zc.super_bitwise_and((mask_blue, mask_blue_n1, mask_blue_n3, mask_lego_u_edge_norm_L))
    if 'lego_only_color' in display_list:
        ...
        zc.check_and_display('lego_only_color', img_color, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    ...
    mask_lego_full = zc.super_bitwise_or((mask_green, mask_yellow, mask_red, mask_blue, mask_lego_u_edge_S))
    mask_lego_full, _ = zc.find_largest_CC(mask_lego_full, min_area = config.BOARD_RECONSTRUCT_AREA / 300.0, min_convex_rate = 0.2, ref_p = config.BOARD_RECONSTRUCT_CENTER, max_dist_ref_p = config.BOARD_RECONSTRUCT_PERI / 15.0)
    ...
    zc.check_and_display('lego_full', img_lego_full, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    ...
    zc.check_and_display('lego', img_lego, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
...
def _find_lego_noboard(img, stretch_ratio, display_list):
    '''
    Find lego without the help of board (e.g. put the lego pieces directly on the table.
    Not finished.
    '''
    bw = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(bw, 50, 100, apertureSize = 3)
    zc.check_and_display('edge', edges, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
    ...
    zc.check_and_display('edge_inv', mask_lego, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
...
def _correct_orientation(img_lego, img_lego_full, display_list):
    ...
    zc.check_and_display('lego_correct', img_lego_correct, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
...
def _get_rectangular_area(img_board, img_correct, rotation_mtx, display_list):
    ...
    zc.check_and_display('lego_rect', img_lego_rect, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_max = config.DISPLAY_MAX_PIXEL, save_image = config.SAVE_IMAGE)
                    ...
                    for i in xrange(n_rows):
                        ...
                        for j in xrange(n_cols):
                            ...
                            n_good_pixels_block = color_cumsum[i_end, j_end] \
                                                ...
                                                + color_cumsum[i_start, j_start]
                            color_cumsum = color_cumsums['unsure']
                            n_good_pixels_block += (color_cumsum[i_end, j_end] \
                                                  - color_cumsum[i_start, j_end] \
                                                  - color_cumsum[i_end, j_start] \
                                                  + color_cumsum[i_start, j_start]) / 2.0 # unsure pixels are half right
...
@config.profile()
def _reconstruct_lego(img_lego, img_board, img_board_ns, rotation_mtx, display_list):
    ...
    img_lego, borders = _lego_outof_board(mask_lego, img_board, rotation_mtx, None)
    img_lego_n0, borders = _lego_outof_board(mask_lego, img_board_n0, rotation_mtx, borders)
    img_lego_n1, borders = _lego_outof_board(mask_lego, img_board_n1, rotation_mtx, borders)
    img_lego_n2, borders = _lego_outof_board(mask_lego, img_board_n2, rotation_mtx, borders)
    img_lego_n3, borders = _lego_outof_board(mask_lego, img_board_n3, rotation_mtx, borders)
    img_lego_n4, borders = _lego_outof_board(mask_lego, img_board_n4, rotation_mtx, borders)
    img_lego_n5, borders = _lego_outof_board(mask_lego, img_board_n5, rotation_mtx, borders)
    img_lego_n6, borders = _lego_outof_board(mask_lego, img_board_n6, rotation_mtx, borders)
    zc.check_and_display('lego_cropped', img_lego, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_scale = config.DISPLAY_SCALE, save_image = config.SAVE_IMAGE)
    ...
    mask_green_on = zc.super_bitwise_and((mask_green[1], mask_green_n1[1], mask_green_n3[1]))
    mask_yellow_on = zc.super_bitwise_and((mask_yellow[1], mask_yellow_n1[1], mask_yellow_n3[1]))
    mask_red_on = zc.super_bitwise_and((mask_red[1], mask_red_n1[1], mask_red_n3[1]))
    mask_blue_on = zc.super_bitwise_and((mask_blue[1], mask_blue_n1[1], mask_blue_n3[1]))
    mask_green_all = zc.super_bitwise_and((mask_green[0], mask_green_n1[0], mask_green_n3[0]))
    mask_yellow_all = zc.super_bitwise_and((mask_yellow[0], mask_yellow_n1[0], mask_yellow_n3[0]))
    mask_red_all = zc.super_bitwise_and((mask_red[0], mask_red_n1[0], mask_red_n3[0]))
    mask_blue_all = zc.super_bitwise_and((mask_blue[0], mask_blue_n1[0], mask_blue_n3[0]))
    mask_colors = zc.super_bitwise_or((mask_green_all, mask_yellow_all, mask_red_all, mask_blue_all))
    ...
    white, green, red, yellow, blue, black = zc.mask2bool((mask_white, mask_green_on, mask_red_on, mask_yellow_on, mask_blue_on, mask_black))
    nothing = np.bitwise_and(np.bitwise_and(img_lego[:,:,0] == 0, img_lego[:,:,1] == 0), img_lego[:,:,2] == 0)
    black = np.bitwise_and(black, np.invert(nothing))
    unsure = np.invert(zc.super_bitwise_or((nothing, white, green, red, yellow, blue, black)))
    ...
    if 'lego_color' in display_list:
        ...
        zc.check_and_display('lego_color', lego_color, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_scale = config.DISPLAY_SCALE, save_image = config.SAVE_IMAGE)

    ## real stuff begins...
    height, width, _ = img_lego.shape
    #print "Expected rows and cols: %f, %f" % (height / config.BRICK_HEIGHT, width / config.BRICK_WIDTH)
    # calculate candidate number of rows and columns for the Lego model, which is purely based on the pixel numbers
    ...
    # the real number of rows and columns may be slightly different, but we don't consider it now...
    for n_rows in xrange(n_rows_opt - 0, n_rows_opt + 1):
        for n_cols in xrange(n_cols_opt - 0, n_cols_opt + 1):
            bitmap, ratio, img_plot, _ = _img2bitmap(img_lego, color_cumsums, n_rows, n_cols, lego_color)
            if bitmap is None:
                continue
    ...
    if best_bitmap is None or best_ratio < 0.85 or best_bitmap.shape != (n_rows_opt, n_cols_opt):
        ...
        return (rtn_msg, None)
    zc.check_and_display('plot_line', best_plot, display_list, wait_time = config.DISPLAY_WAIT_TIME, resize_scale = config.DISPLAY_SCALE, save_image = config.SAVE_IMAGE)
...
def process(img, stretch_ratio, display_list):
    ######################## detect board ######################################
    out = _locate_board(img, stretch_ratio, display_list)
    rtn_msg = out[0]
    extra = out[1:]
    if rtn_msg['status'] != 'success':
        return (rtn_msg, None)
    ...
    rtn_msg, objects = _correct_orientation(img_lego, img_lego_full, display_list)
    if objects is not None:
